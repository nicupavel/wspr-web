 ...
 *
 * Modified for Leaflet 1.9.x from https://github.com/ha8tks/Leaflet.Maidenhead/tree/master
 *
 */

import L from 'leaflet'

export const useMaidenheadLeaflet = () => {
  L.Maidenhead = L.LayerGroup.extend({
    ...
    redraw: function () {
      ...
      for (var lon = left; lon < right; lon += unit * 2) {
        for (var lat = bottom; lat < top; lat += unit) {
          ...
          //console.log(pont.x);
          this.addLayer(
            this._getLabel(lon + unit - unit / lcor, lat + unit / 2 + (unit / lcor) * c)
          )
        }
      }
      return this
    ...
    _getLabel: function (lon, lat) {
      ...
      var title =
        '<span style="cursor: default;"><font style="color:' +
        this.options.color +
        '; font-size:' +
        size +
        '; font-weight: 900; ">' +
        this._getLocator(lon, lat) +
        '</font></span>'
    ...
    _getLocator: function (lon, lat) {
      ...
      for (var i = 0; i < 4; i = i + 1) {
        if (precision > i + 1) {
          ...
          if (i % 2 == 0) {
            locator +=
              Math.floor(rlon / (ydiv_arr[i + 1] * 2)) + '' + Math.floor(rlat / ydiv_arr[i + 1])
          } else {
            locator +=
              d2[Math.floor(rlon / (ydiv_arr[i + 1] * 2))] +
              '' +
